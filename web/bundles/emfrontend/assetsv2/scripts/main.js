(function ($) {
	...
	$(document).ready(function () {
		...
		for (var i = 0; i <= 12; i++) $('.items').append($("<option />").val(i).text(i));
		$('.items').append($("<option />").val("").text("Autres"));
		...
		//$('.table').DataTable();
		$('a.dfilter').on('click', function (e) {
			e.preventDefault();

			var item = $(this).data("filter");
			$('.dfilter').removeClass('active');
			$(this).addClass('active');
			$('.ul-search-result').hide();
			$('.' + item).show(true);
		});

		if ($('.table-bien').length) {
			$('.table-bien').DataTable();
		}
